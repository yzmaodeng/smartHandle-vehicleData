...
@Repository("jCCXCertificateCompressDao")
public class JCCXCertificateCompressDao {
	private static final Log logger = LogFactory.getLog(JCCXCertificateCompressDao.class);
	...
	public void batchInsertData(final HSSFSheet sheetAt) {
		String s = "    SELECT" + "      count(*)" + "      " + "    FROM" + "     INFORMATION_SCHEMA.COLUMNS"
				+ "    where" + "    table_schema ='commercail_zl'" + "    AND" + "    table_name  = 'cerTableJCCX'";
		Integer num = 0;
		try {
		...
		stringBuilder.append("insert into cerTableJCCX values(");
		...
		// 存在空白行的情况的处理
				...
				String ss = "delete from cerTableJCCX where catarc_card_id='';";
...
//	String s1="select count(*) from cerTableJCCX   WHERE xbx_seq_id=''";
//		
//		
...
//		}
		Long HGZId = null;
		String s = "SELECT catarc_card_id from cerTableJCCX";
		List<String> queryForList = jdbcTemplate.queryForList(s, String.class);
		String ss = " update cerTableJCCX set xbx_seq_id=? where catarc_card_id=?";
		int i = 0;
	...
	public void emission_standard() {
		...
		final String pmapsql = "select emission_standard,catarc_card_id from cerTableJCCX where emission_standard!=''";
		...
		for (Map<String, Object> pmap : PList) {
			...
			if (pV) {
				updatePVsql = " update cerTableJCCX set emission_standard='国五' where catarc_card_id= '" + pid + "'";
			} else if (pIV) {
				updatePVsql = " update cerTableJCCX  set emission_standard='国四' where catarc_card_id=  '" + pid + "'";
			} else {
				continue;
			}
		...
		String fixsqlIv = "update cerTableJCCX  set  emission_standard='国四' where  emission_standard  like '%Ⅳ%' or emission_standard  like '%IV%'  ;";
		jdbcTemplate.execute(fixsqlIv);
		logger.info("漏掉的國四已經處理");

		String fixsqlV = "update cerTableJCCX  set  emission_standard='国五' where  emission_standard  like '%v%' or emission_standard  like '%V%' or  emission_standard  like '%Ⅴ%';";
		...
		String fixsqe = "update cerTableJCCX set emission_standard='' where   fuel_type like '%电%'";
		jdbcTemplate.execute(fixsqe);
		logger.info("将电车的排放标准都变成空串");
		
		String fixiv = "update cerTableJCCX set emission_standard='国Ⅴ'  where   emission_standard='国五'";
		jdbcTemplate.execute(fixiv);
		logger.info("将国v的按照字典标准化");
		
		
		String fixv = "update cerTableJCCX set  emission_standard='国Ⅳ' where  emission_standard='国四'";
	...
	public List<Map<String, Object>> getIdCompressIdsJCCX() {
		// String s1="select count(*) from cerTableJCCX WHERE
		// catarc_card_ref_ids=''";
		//
		//
		// Integer catarc_card_idnull = jdbcTemplate.queryForObject(s1,
		// Integer.class);
		//
		// if(catarc_card_idnull.intValue()==0)
		// {
		// return null;
		// }
		String s = "select max(catarc_card_id) as catarc_card_id,group_concat(catarc_card_id separator '|') as catarc_card_ref_ids from cerTableJCCX c GROUP BY "
	...
	public void updateCatarc(final List<Map<String, Object>> idmap) {
		...
		String sql = "";
		sql = "update  cerTableJCCX set catarc_card_ref_ids=  ?  where catarc_card_id= ? ";
		...
		String sql2 = "delete from cerTableJCCX where catarc_card_ref_ids=''";
	...
	public void checkTable() {
		...
		String s = "drop table if exists cerTableJCCX;";
		String s1 = "drop table if exists cerTableJCCX_;";
		try {
			jdbcTemplate.execute(s);
	...
	public void importCertificate(int i) {
		...
		jdbcTemplate.execute("Truncate Table certificate_jccx");



		String importSql = "insert into certificate_jccx "
		...
		if (i == 1) {
			...
			updateCategory = "update certificate_jccx set  vehicle_category='客车'";
		} else {
			updateCategory = "update certificate_jccx set  vehicle_category='货车'";
		}
	...
	public void changeCertificateDefautValue() {
		...
		for (Map<String, Object> map : queryForList) {
			...
			if (zdm.indexOf("status") != -1) {
				SQL = "update  cerTableJCCX  set " + zdm + "='0' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
				jdbcTemplate.execute(SQL);
				continue;
			}
			if (sjlx.indexOf("int") != -1 || sjlx.startsWith("float")) {
				SQL = "update  cerTableJCCX  set " + zdm + "='-1' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
			} else if (sjlx.startsWith("varchar")) {
				SQL = "update  cerTableJCCX  set " + zdm + "='no' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
	...
	public void updateData() {
		...
		String s1 = "update cerTableJCCX  set create_time='" + format + "' , update_time ='" + format + "'";
		try {
			jdbcTemplate.execute(s1);
		} catch (Exception e) {
			logger.info("时间更新失败！！！！");
	...
	public List<Map<String, Object>> getMfgrBrand() {
		String sql = "select mfgr_name_full, brand_name, catarc_card_id from cerTableJCCX";
...
Integer queryForObject = jdbcTemplate.queryForObject(sql, Integer.class);
	...
	public List<Map<java.lang.String, Object>> queyCompressClumns() {
		...
		String sql = " select  gb_code,engine_type , (select id from dict_base_info where name=emission_standard and status=0 and type=33 )  emission_standard"
				...
				+ "if(capacity_rated = 'no','',capacity_rated)capacity_rated,wheelbase,catarc_card_id,catarc_card_ref_ids from certificate_jccx ";
	...
	public List<Map<String, Object>> getNumCompressColum(Map<String, Object> map) {
		...
		try {
			...
			e.printStackTrace();
			logger.info("和老数据匹配出现问题getNumCompressColum！！！！");
			logger.info(e.getMessage());
			System.exit(0);
		}
		return queryForList;
	...
	public void cancleCertificate(Long catarc_card_id, String string) {
		...
		String s = "UPDATE  certificate_jccx   SET  modelIDs = ? ,status =?  WHERE  catarc_card_id =  ?";
	...
	* @author 臧亮 
	* @throws
	* @date 2018年4月2日 下午7:14:21 
	* @version V1.0   
	*/
	public List<java.lang.String> erportErrorEmissionStandard() {
		String s ="select  distinct(emission_standard)  from cerTableJCCX where  emission_standard!='国Ⅳ' and  emission_standard!='国Ⅴ' and  emission_standard!=''";
		List<String> queryForList = jdbcTemplate.queryForList(s, String.class);
		return queryForList;
	}
	...
	* @author 臧亮 
	* @throws
	* @date 2018年4月3日 下午5:08:52 
	* @version V1.0   
	*/
	public void deleteTop() {


		String s ="delete   from cerTableJCCX  LIMIT 1;";
		jdbcTemplate.execute(s);
	...
	/** 
	* @Title: copycerTableJCCX 
	* @Description: TODO(这里用一句话描述这个方法的作用) 
	* @param     入参
	* @return void    返回类型
	* @author 臧亮 
	* @throws
	* @date 2018年4月8日 下午6:40:06 
	* @version V1.0   
	*/
//	public void copycerTableJCCX() {
//		String s="CREATE TABLE cerTableJCCX_ SELECT * FROM cerTableJCCX ";
//		jdbcTemplate.execute(s);
//	}
	...
	*/
	public void restoreStaData() {
		...
		String s1 = "drop table if exists cerTableJCCX;";
		String s="CREATE TABLE cerTableJCCX SELECT * FROM cerTableJCCX_ ";
		jdbcTemplate.execute(s1);
		jdbcTemplate.execute(s);
	...
	* @author 臧亮 
	* @throws
	* @date 2018年4月13日 下午6:12:27 
	* @version V1.0   
	*/
	public void updategbCodeFn() {
		
		
		String s1="update certificate_jccx   set gb_code_fn = gb_code;";
		jdbcTemplate.execute(s1);
		String s2="update certificate_jccx  " +
		" " +
		"set gb_code_fn = (  " +
		"case   " +
		...
		String s7="update certificate_jccx set gb_code_fn = SUBSTRING(gb_code_fn  , 1, 1)";
