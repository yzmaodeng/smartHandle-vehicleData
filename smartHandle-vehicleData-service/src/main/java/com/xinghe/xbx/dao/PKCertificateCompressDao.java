...
@Repository("pkCertificateCompressDao")
public class PKCertificateCompressDao {
	private static final Log logger = LogFactory.getLog(PKCertificateCompressDao.class);
	...
	public void batchInsertData(final HSSFSheet sheetAt) {
		String s = "    SELECT" + "      count(*)" + "      " + "    FROM" + "     INFORMATION_SCHEMA.COLUMNS"
				+ "    where" + "    table_schema ='commercail_zl'" + "    AND" + "    table_name  = 'cerTablePK'";
		...
		stringBuilder.append("insert into cerTablePK values(");
		...
		String ss = "delete from cerTablePK where catarc_card_id='';";
	...
	public void createHGZID() {
		Long HGZId = null;
		String s = "SELECT catarc_card_id from cerTablePK";
		List<String> queryForList = jdbcTemplate.queryForList(s, String.class);
		String ss = " update cerTablePK set xbx_seq_id=? where catarc_card_id=?";
		int i = 0;
	...
	public void emission_standard() {
		...
		final String pmapsql = "select emission_standard,catarc_card_id from cerTablePK where emission_standard!=''";
		...
		for (Map<String, Object> pmap : PList) {
			...
			if (pV) {
				updatePVsql = " update cerTablePK set emission_standard='国五' where catarc_card_id= '" + pid + "'";
			} else if (pIV) {
				updatePVsql = " update cerTablePK  set emission_standard='国四' where catarc_card_id=  '" + pid + "'";
			} else {
				continue;
			}
		...
		String fixsqlIv = "update cerTablePK  set  emission_standard='国四' where  emission_standard  like '%Ⅳ%' or emission_standard  like '%IV%'  ;";
		jdbcTemplate.execute(fixsqlIv);
		logger.info("漏掉的國四已經處理");

		String fixsqlV = "update cerTablePK  set  emission_standard='国五' where  emission_standard  like '%v%' or emission_standard  like '%V%' or  emission_standard  like '%Ⅴ%';";
		jdbcTemplate.execute(fixsqlV);
		logger.info("漏掉的國五已經處理");
		String fixsqe = "update cerTablePK set emission_standard='' where   fuel_type='电'";
		jdbcTemplate.execute(fixsqlV);
		logger.info("将电车的排放标准都变成空串");

		String fixiv = "update cerTablePK set emission_standard='国Ⅴ'  where   emission_standard='国五'";
		jdbcTemplate.execute(fixiv);
		logger.info("将国v的按照字典标准化");

		String fixv = "update cerTablePK set  emission_standard='国Ⅳ' where  emission_standard='国四'";
	...
	public List<Map<String, Object>> getIdCompressIds() {
		...
		String s = "select max(catarc_card_id) as catarc_card_id,group_concat(catarc_card_id separator '|') as catarc_card_ref_ids from cerTablePK c GROUP BY c.gb_code,c.engine_type ,"
	...
	public List<Map<String, Object>> updateCompressDataPK() {
		...
		String s = "select max(catarc_card_id) as catarc_card_id,group_concat(catarc_card_id separator '|') as catarc_card_ref_ids from cerTablePK c GROUP BY "
	...
	public void updateCatarc(final List<Map<String, Object>> idmap) {
		// String s = "select count(*) from cerTablePK WHERE catarc_card_id=''";
		// Integer catarc_card_idnull = jdbcTemplate.queryForObject(s,
		// Integer.class);
		// if (catarc_card_idnull.intValue() == 0) {
		// return;
		// }
		String sql = "";
		sql = "update  cerTablePK set catarc_card_ref_ids=  ?  where catarc_card_id= ? ";
		...
		String sql2 = "delete from cerTablePK where catarc_card_ref_ids='' ";
	...
	public void checkTable() {
		...
		String s = "drop table if exists cerTablePK;";
		String s1 = "drop table if exists cerTablePK_;";
		try {
			jdbcTemplate.execute(s);
	...
	public void importCertificate(int i) {
		...
		jdbcTemplate.execute("Truncate Table certificate_pk");

		String importSql = "insert into certificate_pk "
	...
	public void changeCertificateDefautValue() {
		...
		for (Map<String, Object> map : queryForList) {
			...
			if (zdm.indexOf("status") != -1) {
				SQL = "update  cerTablePK  set " + zdm + "='0' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
				jdbcTemplate.execute(SQL);
				continue;
			}
			if (sjlx.indexOf("int") != -1 || sjlx.startsWith("float")) {
				SQL = "update  cerTablePK  set " + zdm + "='-1' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
			} else if (sjlx.startsWith("varchar")) {
				SQL = "update  cerTablePK  set " + zdm + "='no' where  " + zdm + "='' or " + zdm + " is null or " + zdm
						+ "=''";
	...
	public void updateData() {
		...
		String s1 = "update cerTablePK  set create_time='" + format + "' , update_time ='" + format + "'";
		try {
			jdbcTemplate.execute(s1);
		} catch (Exception e) {
			logger.info("时间更新失败！！！！");
	 ...
	 * @author 臧亮
	 * @throws @date
	 *             2018年3月30日 上午11:09:18
	 * @version V1.0
	 */
	public List getLackBrand() {
	 ...
	 * @author 臧亮
	 * @throws @date
	 *             2018年3月30日 上午11:09:27
	 * @version V1.0
	 */
	public List getLackMfgr() {

		return null;
	}

	public List<Map<String, Object>> getMfgrBrand() {
		String sql = "select mfgr_name_full, brand_name, catarc_card_id from cerTablePK";
	...
	public List<Map<java.lang.String, Object>> queyCompressClumns() {
		...
		String sql = " select  gb_code,engine_type , (select id from dict_base_info where name=emission_standard and status=0 and type=33 )  emission_standard"
				...
				+ " wheelbase,catarc_card_id,catarc_card_ref_ids from certificate_pk ";
	...
	public List<Map<String, Object>> getNumCompressColum(Map<String, Object> map) {
		...
		try {
			final String SQL = "select model_id,catarc_card_id,catarc_card_ref_ids from "
					...
					+ " AND weight_curb= ? AND ex_factory_time= ? AND drive_form_id= ? AND wheelbase= ? ";
			...
			Object[] params = new Object[] { gb_code, engine_type, emission_standard, power_max, displacement_ML,
					...
					weight_curb, ex_factory_time, drive_form, wheelbase };
	 ...
	 * @version V1.0
	 * @param catarc_card_id_old
	 */
	public void cancleCertificate(Long catarc_card_id, String string) {

		String s = "UPDATE  certificate_pk   SET  modelIDs = ? ,status =?  WHERE  catarc_card_id =  ?";
	...
	public List<java.lang.String> erportErrorEmissionStandard() {
		String s = "select  distinct(emission_standard)  from cerTablePK where  emission_standard!='国Ⅳ' and  emission_standard!='国Ⅴ' and  emission_standard!=''";
		List<String> queryForList = jdbcTemplate.queryForList(s, String.class);
		return queryForList;
	}
	...
	public void deleteTop() {
		...
		String s = "delete   from cerTablePK  LIMIT 1;";
		jdbcTemplate.execute(s);
	 ...
	 * @author 臧亮
	 * @throws @date
	 *             2018年4月9日 上午10:44:03
	 * @version V1.0
	 */
	public void duplicateStaData() {
		String s = "CREATE TABLE cerTablePK_ SELECT * FROM cerTablePK ";
		jdbcTemplate.execute(s);
	 ...
	 * @author 臧亮
	 * @throws @date
	 *             2018年4月9日 上午11:25:24
	 * @version V1.0
	 */
	public void restoreStaData() {

		String s1 = "drop table if exists cerTablePK;";
		String s = "CREATE TABLE cerTablePK SELECT * FROM cerTablePK_ ";
		jdbcTemplate.execute(s1);
		jdbcTemplate.execute(s);
	...
	*/
	...
	public void updategbCodeFn() {
		...
		String s1="update certificate_pk   set gb_code_fn = gb_code;";
		jdbcTemplate.execute(s1);
		String s2="update certificate_pk  " +
		" " +
		"set gb_code_fn = (  " +
		"case   " +
		...
		String s7="update certificate_pk set gb_code_fn = SUBSTRING(gb_code_fn  , 1, 1)";
